...
class HStoreConstraint():
    ...
    value_operators = {'exact': '=', 'iexact': '=', 'in': 'IN',
                       'lt': '<', 'lte': '<=', 'gt': '>', 'gte': '>='}
    ...
    def __init__(self, alias, field, value, lookup_type, key=None):
        ...
        if lookup_type == 'contains':
            ...
            if cast_type:
                self.lvalue = "CAST(NULLIF(%%s->'%s','') AS %s)" % (key,
                                                                    cast_type)
...
class HQ(tree.Node):
    ...
    OR = 'OR'
    default = AND
    query_terms = ['exact', 'iexact', 'lt', 'lte', 'gt', 'gte',
                   'in', 'contains']
        ...
        for child in self.children:
            if  isinstance(child, HQ):
                ...
                if len(parts) > 1 and parts[-1] in self.query_terms:
                    # Traverse the lookup query to distinguish related fields
                    # from lookup types.
                    ...
                    for counter, field_name in enumerate(parts):
                        try:
                            ...
                            break
                        # Unless we're at the end of the list of lookups,
                        # let's attempt to continue traversing relations.
                ...
                field, target, opts, join_list, last, extra = query.setup_joins(
                    parts, opts, alias, True)
...
def add_hstore(queryset, field, key, name=None):
    assert queryset.query.can_filter(), "Cannot change a query once a slice " \
                                        "has been taken"
    name = name or key
    clone = queryset._clone()
    clone.query.add_extra({name: "%s -> '%s'" % (field, key)},
