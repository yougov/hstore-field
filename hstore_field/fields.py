        ...
        if connection.connection.server_version < 90000:
            ...
            with open(hstore_sql, 'U') as fp:
                for statement in statements.split(
                        fp.read().decode(settings.FILE_CHARSET)):
                    statement = re.sub(ur"--.*([\n\Z]|$)", "",
                                       statement).strip()
                    if statement:
...
connection_created.connect(register_hstore_on_connection_creation,
    dispatch_uid='hstore_field.register_hstore_on_connection_creation')
